...
def get_activities(user_id):
    activities_df = pandas.DataFrame([(activity.activity_date, activity.sport.label) for
                                      activity in
                                      Activity.objects.all().order_by('-activity_date').filter(
                                          user_id=user_id)[:10]], columns=['Date', 'Sport'])
...
class UserAllActivitiesList(APIView):

    authentication_classes = (authentication.SessionAuthentication,)
    permission_classes = (permissions.IsAuthenticated,)

    def get(self, request):
        return Response(get_activities(request.user.id))
...
class UserActivitiesList(APIView):
    ...
    def get(self, request, user_id):
        return Response(get_activities(user_id))
